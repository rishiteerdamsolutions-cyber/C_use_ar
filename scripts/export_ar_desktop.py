...
def _write_spec(build_dir: Path, repo: Path, exe_name: str) -> Path:
    ...
    datas = [
        (str(repo / "TRAINER.html"), "."),
        (str(repo / "dashboard.py"), "."),
    ]
    if env_ex.is_file():
...
def run_export(
    *,
    ...
    # Ship one bundle + workflows beside the one-folder executable (desktop.py AGENCY_HOME).
    ...
    env_lines = [
        ...
        "",
        "# End user: set daily time + topics under Automation (per workflow) and ar™ schedule under ar™.",
    ]
    if embed_api_keys:
        for key in (
            "OPENAI_API_KEY",
    ...
    readme = textwrap.dedent(
        ...
        • Schedule: use the **ar™** tab for bundle daily time; use **automation** per workflow for topic queues / campaign days (e.g. 30 topics over 30 days). Pause or change counts there, then save.
        • **Restart** automation: automation tab → adjust topics/days or clear in-progress fields via workflow JSON editor in Trainer if needed; re-save schedule.
